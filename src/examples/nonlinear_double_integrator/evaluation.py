...
from nonlinear_double_integrator import NonlinearDoubleIntegrator, NLP, NLPCasadi, NonlinearDoubleIntegratorCasadi, NMPCCasadi
from models import FeedforwardNN, Predictor, ApproxMPC, Solver, export_jit_cpu, export_jit_gpu
...
# Functions
NUM_ITERATIONS = 100
SOLVER_EVAL_ITERATIONS = [0,10,100,200]
TOL = 1e-6
# UPDATE_MODE = "full"
...
def load_predictor(pth):
    if pth is None:
        ...
        prd_cfg = load_config(pth.joinpath("config.json"))
...
def load_approx_mpc(pth):
    appr_cfg = load_config(pth.joinpath("config.json"))
    model_cfg = appr_cfg["model_cfg"]
    
    # Scaling factors
    approx_mpc_config = appr_cfg["approxMPC_cfg"]
    scale_data = approx_mpc_config["scale_data"]
    if scale_data:
        bounds = {"x_lb": approx_mpc_config["bounds"]["x_lb"],
                    "x_ub": approx_mpc_config["bounds"]["x_ub"],
                    "u_lb": approx_mpc_config["bounds"]["u_lb"],
                    "u_ub": approx_mpc_config["bounds"]["u_ub"]}
        for key in bounds.keys():
            bounds[key] = torch.tensor(bounds[key],dtype=DTYPE,device=DEVICE)
    else:
        bounds = None

    model = FeedforwardNN(**model_cfg)
    # model = model.to(dtype=DTYPE,device=DEVICE)
    approx_mpc = ApproxMPC(model,scale_data=scale_data,bounds=bounds,torch_compiled=False)
    approx_mpc.load_weights(pth,file_name="approx_mpc_weights.pt")
    return approx_mpc,appr_cfg
...
def evaluate_approx_mpc(approx_mpc,test_data,clip_outputs=False,speed_eval=True,system=None):
    # 1. load data and data metrics
    x0_batch = test_data["x0"].to(DTYPE)
    u0_opt_batch = test_data["u0"].to(DTYPE)

    # 2. predict
    # scale_data = approx_mpc.scale_data
    u0_pred_batch = approx_mpc.make_step(x0_batch,clip_outputs=clip_outputs)
       
    # 3. evaluate
    # distance to data
    u0_diff_batch = u0_pred_batch - u0_opt_batch
    u0_abs_diff_batch = torch.abs(u0_diff_batch)
    distance_dict = get_metrics(u0_abs_diff_batch,var_name="u0_abs_diff")

    # 4. inference speed
    if speed_eval:
        model_jit_gpu = export_jit_gpu(approx_mpc.model)
        model_jit_cpu = export_jit_cpu(approx_mpc.model)
        # 4.1 batch prediction GPU
        inference_times_batch_gpu = measure_inference_time_predictor(model_jit_gpu, x0_batch, use_cuda=True)
    ...
    # 5. one-step feasibility check
    feas_results = get_feasibility_results(x0_batch, u0_pred_batch,check_recursive_feasibility=False,system=system)

    # 6. stack results
    results_dict = {**distance_dict,**feas_results}
    if speed_eval:
    ...
    results_dict.update({"u0_abs_diff": u0_abs_diff_batch.cpu().numpy().squeeze()})

    return results_dict

def get_feasibility_results(x0_batch, u0_pred_batch, check_recursive_feasibility=False, system=None):
    if system is None:
        system = NonlinearDoubleIntegrator(device=DEVICE, dtype=DTYPE)
    
    # Original version
    u0_pred_batch_apply = u0_pred_batch.clone()
    x1_batch = system.parallel_step(x0_batch, u0_pred_batch_apply)
    
    # Vectorized constraint checking (now fast!)
    x_batch_ok, u_batch_ok = system.check_constraints_batch(x1_batch, u0_pred_batch_apply)
    
    # Efficient fraction computation
    # x_batch_ok is [batch, N], u_batch_ok is [batch]
    if x_batch_ok.dim() == 2:
        # For trajectory: count violations across batch AND time
        frac_x1_infeasible = (~x_batch_ok).float().mean().item()
    else:
        # For single timestep
        frac_x1_infeasible = (~x_batch_ok).float().mean().item()
    
    frac_u0_infeasible = (~u_batch_ok).float().mean().item()
    
    # Clipped version
    u0_pred_batch_apply = torch.clamp(u0_pred_batch, system.u_lb, system.u_ub)
    x1_batch_clip = system.parallel_step(x0_batch, u0_pred_batch_apply)
    
    x_batch_ok_clip, u_batch_ok_clip = system.check_constraints_batch(x1_batch_clip, u0_pred_batch_apply)
    
    if x_batch_ok_clip.dim() == 2:
        frac_x1_infeasible_clip = (~x_batch_ok_clip).float().mean().item()
    else:
        frac_x1_infeasible_clip = (~x_batch_ok_clip).float().mean().item()
    
    frac_u0_infeasible_clip = (~u_batch_ok_clip).float().mean().item()

    if check_recursive_feasibility:
        raise NotImplementedError("Recursive feasibility check not implemented due to extreme slow speed.")
    
    return {
        "frac_x1_infeasible": frac_x1_infeasible,
        "frac_u0_infeasible": frac_u0_infeasible,
        "frac_x1_infeasible_clip": frac_x1_infeasible_clip,
        "frac_u0_infeasible_clip": frac_u0_infeasible_clip,
    }

def evaluate_predictions(z_pred_batch,test_data,nlp):
    # 1. load data and data metrics
    p_batch = test_data["x0"].to(DTYPE)
    u0_opt_batch = test_data["u0"].to(DTYPE)
    ...
    X_pred_batch, U_pred_batch = nlp.extract_states_controls_batch(w_pred_batch)
    u0_pred_batch = U_pred_batch[:,0,:]
    ...
    # 3.1.3 stack results
    optimality_dict = {**KKT_inf_results,**Tk_results,**KKT_2_results}

    # 3.2 distance to data
    u0_diff_batch = u0_pred_batch - u0_opt_batch
    u0_abs_diff_batch = torch.abs(u0_diff_batch)
    u0_abs_results = get_metrics(u0_abs_diff_batch,var_name="u0_abs_diff")
    ...
    z_diff_norm_batch = torch.linalg.vector_norm(z_diff_batch,dim=1)
    w_diff_norm_batch = torch.linalg.vector_norm(w_diff_batch,dim=1)
    nu_diff_norm_batch = torch.linalg.vector_norm(nu_diff_batch,dim=1)
    lam_diff_norm_batch = torch.linalg.vector_norm(lam_diff_batch,dim=1)
    ...
    distance_dict = {**u0_abs_results,**z_diff_norm_results,**w_diff_norm_results,**nu_diff_norm_results,**lam_diff_norm_results}
    ...
    # 3.5 feasibility results
    feas_results = get_feasibility_results(p_batch, u0_pred_batch,check_recursive_feasibility=False,system=nlp.system)

    results_dict = {**optimality_dict, **distance_dict, **constraints_dict, **opt_gap_dict,**feas_results}
...
def evaluate_solver(solver,test_data,solver_cfg,speed_eval=False):
    ...
    trajectory_evaluation_list, gpu_n_iter_max = evaluate_trajectory(zk_traj,p_batch,nlp,test_data=test_data,torch_compiled=True)
    all_results_dict["trajectory_evaluation"] = trajectory_evaluation_list
    all_results_dict["gpu_n_iter_max"] = gpu_n_iter_max
    ...
    if speed_eval:
        ...
        nlp_casadi = NLPCasadi(system_casadi,solver.nlp.N)
        ...
        z_i = z_opt_batch[0,:]
        p_i = p_batch[0,:]
        cpu_step_times = measure_step_time_solver_cpu(solve_step_cpu,Tk_func_cpu, z_i, p_i)

        # GPU inference time
        gpu_step_times = get_metrics(torch.tensor(step_time_list),var_name="gpu_step_time")

        # full solve time compared to IPOPT
        cpu_solver_times_results = measure_full_time_solver_cpu(test_data,solve_step_cpu,Tk_func_cpu,solver,UPDATE_MODE,alpha)
    else:
        cpu_step_times = {}
        gpu_step_times = {}
...
def evaluate_trajectory(zk_traj,p_batch,nlp,test_data=None,torch_compiled=False):
    if torch_compiled:
    ...
    for idx, zk_i in enumerate(zk_traj):
        ...
        if KKT_lim_frac > KKT_lim_frac_prev:
            gpu_n_iter_max = idx
            KKT_lim_frac_prev = KKT_lim_frac
        ...
        if test_data is not None:
            # p_batch = test_data["x0"].to(DTYPE)
            assert torch.allclose(p_batch,test_data["x0"].to(DTYPE))
            u0_opt_batch = test_data["u0"].to(DTYPE)

            w_pred_batch, nu_pred_batch, lam_pred_batch = nlp.extract_primal_dual_batch(zk_i)
            X_pred_batch, U_pred_batch = nlp.extract_states_controls_batch(w_pred_batch)
            u0_pred_batch = U_pred_batch[:,0,:]            
            ...
            u0_abs_diff_batch = torch.abs(u0_diff_batch)
            u0_abs_results = get_metrics(u0_abs_diff_batch,var_name="u0_abs_diff")
            traj_eval.update(u0_abs_results)

            feas_results = get_feasibility_results(p_batch, u0_pred_batch,check_recursive_feasibility=False)
            traj_eval.update(feas_results)

        traj_eval_list.append(traj_eval)
    
    return traj_eval_list, gpu_n_iter_max
...
def visualize_approxMPC(results_dict,pth=None):
    u0_abs_diff = results_dict["u0_abs_diff"]
    # plot a histogram of the absolute difference
    fig, ax = plot_histogram_u0_abs_diff(u0_abs_diff,ax=None,label="ApproxMPC")
    ax.legend()

    # save figure
    if pth is not None:
        fig.savefig(pth.joinpath("approx_mpc_histogram.png"), dpi=300, bbox_inches='tight')
        plt.close(fig)
...
def visualize_predictor(results_dict,pth=None):
    u0_abs_diff = results_dict["u0_abs_diff"]
    u0_abs_diff = u0_abs_diff[u0_abs_diff > 1e-12]
    # plot a histogram of the absolute difference
    fig, ax = plot_histogram_u0_abs_diff(u0_abs_diff,ax=None)
...
def visualize_solver(results_dict,pth=None):
    ...
    ax1 = None
    for i in SOLVER_EVAL_ITERATIONS:
        u0_abs_diff = results_dict[f"solver_{i}"]["u0_abs_diff"]
        # filter all for which u0_abs_diff is <= 1e-12
        u0_abs_diff = u0_abs_diff[u0_abs_diff > 1e-12]
        fig1, ax1 = plot_histogram_u0_abs_diff(u0_abs_diff,ax=ax1,label=f"iter {i}: {len(u0_abs_diff)}")

    ax1.legend()

    # 2. convergence behavior of solver (KKT infinity norm metrics over iterations)
    fig2, ax2 = plot_kkt_convergence(results_dict,ax=None)
    ...
    if pth is not None:
        ...
        fig2.savefig(pth.joinpath("solver_kkt_convergence.png"), dpi=300, bbox_inches='tight')
...
def plot_histogram_u0_abs_diff(u0_abs_diff,ax=None,label=None):
    # settings
    n_bins = 100
    alpha_trans = 0.3

    diff_u_vals = u0_abs_diff.copy()

    # diff_u_vals = diff_u_vals[diff_u_vals > 1e-12]

    # calculate histograms
    log_diff_u0 = np.log10(diff_u_vals+1e-16)
    ...
    if ax is None:
        ...
        fig = ax.figure
    # plot histogram
    ax.plot(diff_u0_hist[1][:-1],diff_u0_hist[0])

    if label is not None:
        ax.hist(log_diff_u0,bins=n_bins,alpha=alpha_trans,label=label)
    else:
        ax.hist(log_diff_u0,bins=n_bins,alpha=alpha_trans)
    ...
    ax.set_xlabel(r"Absolute error to optimal control action $|\hat{u}_0 - u_{0}^{*}|$")
...
def plot_kkt_convergence(results_dict,ax=None):
...
if __name__ == "__main__":
    ...
    run_folder = "results"
    modes = ["approxMPC","predictor","solver"]

    test_data_pth = FILE_PTH.joinpath("data","closed_loop_N_5000_Nsim_5_0","nmpc_data.npz") # - closed loop
    
    for mode in modes:
        pth = FILE_PTH.joinpath(run_folder,mode)
        ...
        assert check_data_integrity(test_data,nlp), "Data integrity check failed!"

        # load configs
        cfg_list = get_all_configs(pth)
        
        if len(cfg_list) == 0:
            raise ValueError(f"No configs found in {pth}.")
        ...
        for cfg in cfg_list:        
            if not cfg["evaluated"] or overwrite:
                ...
                if mode == "approxMPC":
                    approx_mpc_pth = Path(cfg["exp_pth"])
                    approx_mpc, appr_cfg = load_approx_mpc(approx_mpc_pth)
                    # evaluate
                    results_dict = evaluate_approx_mpc(approx_mpc,test_data,clip_outputs=False,speed_eval=speed_eval,system=system)
                    # visualize results
                    if visualize_results:
                        figs = visualize_approxMPC(results_dict,pth=approx_mpc_pth)
